...
def custom_corner_impl(
    ...
    **hist2d_kwargs,
    ...
    axes, new_fig = _get_fig_axes(fig, K, samples=xs, data_labels=labels)
...
def _get_fig_axes(fig, K, samples=None, data_labels=None):
    if not fig.axes:
        return fig.subplots(K, K), True
    try:
        if isinstance(samples, (np.ndarray, dict)):
            ...
            labels = []
            labels.append(axes[-1, 0].get_xlabel())
            for yi in range(1, ndim):
                ax = axes[yi, 0]
                labels.append(ax.get_ylabel())

            if data_labels is None:
                data_labels = [k for k in labels if k in samples.keys()]
                raise NotImplementedError

            # axes_dims = np.array(list(product(labels, labels))).reshape((*axes.shape, 2))
            # axes_dims = axes_dims.view(dtype=np.dtype([('x', axes_dims.dtype), ('y', axes_dims.dtype)]))
            # axes_dims = axes_dims.reshape(axes_dims.shape[:-1])

            mask = [x in data_labels and y in data_labels for x, y in product(labels, labels)]
            mask = np.array(mask).reshape(axes.shape)

            return np.ma.masked_array(axes, mask=~mask).compressed().reshape((len(data_labels), len(data_labels))), False
        else:
